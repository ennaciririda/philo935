...
/*   Updated: 2023/06/01 17:15:35 by rennacir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_table	*init_table(char **split)
{
	...
	i = -1;
	while (++i < table->philo_num)
		init_each_philo(table->philos + i, forks, table, i);
	return (table);
}
...
int	main(int argc, char **argv)
{
	...
	while (split[num])
		num++;
	if (!args_num(num) || !check_is_digit(split)
		|| !check_max_min(split) || !zero_case(split))
	{
		free_all(split);
		return (0);
	}
